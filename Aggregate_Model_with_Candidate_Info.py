...
class AggregateVotingModel(nn.Module):
    ...
    + Candidate-aware term:
      θ_ic,k ∝ exp( γ0_k + b_i^(P,k) + x_ic^T β_P,k + η^T g_ic,k )
    """
    
    def __init__(self, n_features, n_parties, n_booths,
                 candidate_dim: int = 0, device: str = "cpu"):
        ...
        self.alpha0 = nn.Parameter(torch.randn(1, device=device))             # [1]
        self.beta_T = nn.Parameter(torch.randn(n_features, device=device))    # [F]
        self.booth_effects_T = nn.Parameter(torch.randn(n_booths, device=device))  # [I]

        # Party choice model: θ_ic = softmax(γ0 + b_i^(P) + x_ic^T β_P)
        self.gamma0 = nn.Parameter(torch.randn(n_parties, device=device))         # [K]
        self.beta_P = nn.Parameter(torch.randn(n_features, n_parties, device=device))  # [F, K]
        self.booth_effects_P = nn.Parameter(
            torch.randn(n_booths, n_parties, device=device)
        )  # [I, K]

        # --- Candidate affinity term η^T g_ic,k (shared η over parties) ---
        self.candidate_dim = int(candidate_dim)
        if self.candidate_dim > 0:
            # Start from zero so candidate term doesn’t explode at init
            self.eta = nn.Parameter(torch.zeros(self.candidate_dim, device=device))  # [D]
        else:
            self.eta = None

    def forward(self, features, booth_indices, candidate_features=None):
        """Forward pass with optional candidate-aware term.

        Args:
            features:           [B, F]
            booth_indices:      [B]
            candidate_features: [B, K, D] where D = candidate_dim (g_ic,k),
                               or None to fall back to baseline model.
        """
        ...
        turnout_probs = torch.sigmoid(turnout_logits)  # [B]

        # --- Baseline party logits (no candidate term) ---
        # base_logits_ic,k = γ0_k + b_i^(P,k) + x_ic^T β_P,k
        party_linear = torch.matmul(features, self.beta_P)  # [B, K]
        base_logits = (
            self.gamma0.unsqueeze(0).expand(batch_size, -1)  # [B, K]
            + self.booth_effects_P[booth_indices]            # [B, K]
            + party_linear                                   # [B, K]
        )

        # --- Candidate term: η^T g_ic,k ---
        if self.eta is not None and candidate_features is not None:
            cand = candidate_features.to(self.device)        # [B, K, D]
            # (b,k,d) * (d) → (b,k)
            cand_logits = torch.einsum("bkd,d->bk", cand, self.eta)
            party_logits = base_logits + cand_logits
        else:
            party_logits = base_logits

        # Softmax in log-space
        party_log_probs = torch.log_softmax(party_logits, dim=1)  # [B, K]
        party_probs = torch.exp(party_log_probs)                  # [B, K]
...
class ElectoralDataProcessor:
    ...
    def __init__(self):
        ...
        # ---- Candidate-aware configuration ----
        # g_ic,k = [age_sim, rel_match, caste_match]  (D = 3)
        self.candidate_feature_dim = 3

        # Age midpoints for voter cells (your age buckets)
        self.cell_age_midpoints = {
            'Age_18-25': 21.5,
            'Age_26-35': 30.5,
            'Age_36-45': 40.5,
            'Age_46-60': 53.0,
        ...
        # Candidate age buckets (we only care about 30+)
        # Raw candidate ages get bucketed into:
        #   Age_30-45, Age_46-60, Age_60+
        self.candidate_age_midpoints = {
            'Age_30-45': 37.5,
            'Age_46-60': 53.0,
            'Age_60+': 70.0,
        }
        ...
        #   - column names in the booth-level dataframe (e.g. 'BJP_cand_age'), OR
        #   - literal raw values like 53, 'hindu', 'obc' (we'll treat them as
        #     direct candidate attributes and ignore the dataframe for those).
        self.candidate_profile_cols = {
            'BJP': {
                'age':      '53',      # literal age value
                'religion': 'hindu',   # literal religion string
                'caste':    'obc',     # literal caste string
            },
            'Congress': {
                'age':      '53',
                'religion': 'hindu',
            ...
            'AAP': {
                'age':      '37',
                'religion': 'hindu',
                'caste':    'vaishya',  # treated as no caste system
            },
            # No entries for Others / NOTA  they will automatically get g_ic,k = 0
        }

        # Optional: global candidate profiles (override Excel columns when set)
        # Format example:
        #   {
        #     'BJP': {'age': 43, 'religion': 'hindu', 'caste': 'vaishya'},
        #     'Congress': {'age': 52, 'religion': 'hindu', 'caste': 'brahmin'},
        #     'AAP': {'age': 38, 'religion': 'hindu', 'caste': 'obc'},
        #   }
        self.global_candidate_profiles = None
    ...
    # -------- RAW → CANONICAL LABEL HELPERS --------

    def _bucket_candidate_age(self, raw_age):
        """Convert raw age (e.g. 37) to candidate age band."""
        ...
        try:
            ...
            # Allow already-bucketed strings like 'Age_46-60'
            if isinstance(raw_age, str):
                raw_age = raw_age.strip()
                if raw_age in self.candidate_age_midpoints:
                    return raw_age
            return None
    ...
    def _normalize_religion_label(self, raw):
        """Normalize raw religion strings into internal labels."""
    ...
    def _normalize_caste_label(self, raw):
        """Normalize raw caste strings into internal caste labels."""
        if raw is None or not isinstance(raw, str):
            return None
        ...
        mapping = {
            # No caste system
            'no_caste_system': 'Caste_No_caste_system',
            'no_caste': 'Caste_No_caste_system',
            'nocastesystem': 'Caste_No_caste_system',
            'general': 'Caste_No_caste_system',

            # Common categories
            'brahmin': 'Caste_Brahmin',
            'kshatriya': 'Caste_Kshatriya',
            'obc': 'Caste_Obc',
            'sc': 'Caste_Sc',
            'st': 'Caste_St',
            'vaishya': 'Caste_Vaishya',
        }

        return mapping.get(key)

    def _age_midpoint_from_label(self, label: str, is_candidate: bool):
        """Map age band label to midpoint value using the right dict."""
        if label is None or not isinstance(label, str):
            return None
        label = label.strip()
        if is_candidate:
            return self.candidate_age_midpoints.get(label)
        return self.cell_age_midpoints.get(label)

    def set_global_candidate_profiles(self, profiles: dict):
        """Set candidate profiles from code instead of Excel columns.

        Expected format per party (raw values):
            {
              'BJP': {
                  'age': 43,              # or 'Age_46-60'
                  'religion': 'hindu',    # e.g. 'hindu', 'muslim'
                  'caste': 'vaishya',     # e.g. 'brahmin', 'obc', 'sc', 'no_caste_system'
              },
              ...
            }

        Values are normalized to the internal labels used by the model so
        you can just hardcode intuitive strings in main().
        """
        canonical = {}
        if profiles is None:
            self.global_candidate_profiles = None
        ...
        for party, info in profiles.items():
            if info is None:
                continue

            raw_age = info.get('age')
            raw_rel = info.get('religion')
            raw_caste = info.get('caste')

            age_cat = self._bucket_candidate_age(raw_age)
            rel_cat = self._normalize_religion_label(raw_rel)
            caste_cat = self._normalize_caste_label(raw_caste)

            # Skip if absolutely nothing is present
            if age_cat is None and rel_cat is None and caste_cat is None:
                continue
        ...
        self.global_candidate_profiles = canonical if canonical else None

    def _get_candidate_profile_for_party(self, row, party: str):
        """Read and normalize candidate profile for a given party from the row."""
        # If global profiles have been set from code, prefer those
        if self.global_candidate_profiles:
            profile = self.global_candidate_profiles.get(party)
            if profile is not None:
                return profile

        cols = self.candidate_profile_cols.get(party)
        if cols is None:
            return None

        def safe_get(col_or_value):
            """Fetch from dataframe if it's a column name, otherwise treat as literal.

            This lets you hardcode values like '53', 'hindu', 'obc' directly in
            candidate_profile_cols without needing extra columns in Excel.
            """
            if col_or_value is None:
                return None

            # If this looks like a dataframe column name, read from row
            if isinstance(col_or_value, str) and col_or_value in row.index:
                val = row[col_or_value]
                if isinstance(val, float) and np.isnan(val):
                    return None
                return val

            # Otherwise, treat it as a raw literal value
            return col_or_value

        raw_age = safe_get(cols.get('age'))
        raw_rel = safe_get(cols.get('religion'))
        raw_caste = safe_get(cols.get('caste'))

        age_cat = self._bucket_candidate_age(raw_age)
        rel_cat = self._normalize_religion_label(raw_rel)
        caste_cat = self._normalize_caste_label(raw_caste)

        # If absolutely nothing is present, treat as missing candidate
        if age_cat is None and rel_cat is None and caste_cat is None:
            return None

        return {
            'age':      age_cat,   # e.g. 'Age_30-45'
            'religion': rel_cat,   # e.g. 'Religion_Hindu'
            'caste':    caste_cat, # e.g. 'Caste_Vaishya' or 'Caste_No_caste_system'
        }

    def _candidate_similarity_vector(self, cell_axes: dict, cand_profile: dict | None):
        """Compute g_ic,k = [age_similarity, religion_match, caste_match]."""
        if cand_profile is None:
            return [0.0, 0.0, 0.0]

        # 1) Age similarity
        age_cell = self._age_midpoint_from_label(cell_axes['age'], is_candidate=False)
        age_cand = self._age_midpoint_from_label(cand_profile.get('age'), is_candidate=True)
        if age_cell is None or age_cand is None:
            age_sim = 0.0
        else:
            diff = abs(age_cell - age_cand)
            tau = 15.0  # tolerance in years
            age_sim = float(np.exp(-diff / tau))

        # 2) Religion exact match
        rel_match = 1.0 if cand_profile.get('religion') == cell_axes['religion'] else 0.0

        # 3) Caste match only meaningful for Hindus
        if cell_axes['religion'] == 'Religion_Hindu':
            caste_match = 1.0 if cand_profile.get('caste') == cell_axes['caste'] else 0.0
        else:
            caste_match = 0.0

        return [age_sim, rel_match, caste_match]

    def _compute_candidate_affinity_for_cell(self, row, cell: dict):
        """Build g_ic,k for all parties for a single cell.

        cell: {'age_cat', 'religion_cat', 'caste_cat', 'income_cat', 'n_ic'}
        Returns: dict party -> list[3]
        """
        cell_axes = {
            'age':      cell['age_cat'],       # e.g. 'Age_46-60'
            'religion': cell['religion_cat'],  # e.g. 'Religion_Hindu'
            'caste':    cell['caste_cat'],     # e.g. 'Caste_Vaishya'
        }

        affinity = {}
        for party in self.party_names:
            cand_profile = self._get_candidate_profile_for_party(row, party)
            affinity[party] = self._candidate_similarity_vector(cell_axes, cand_profile)
        ...
        for idx, row in df.iterrows():
            ...
            if valid_cells:
                ...
                for cell in valid_cells:
                    ...
                    # NEW: candidate affinity g_ic,k for this cell (per party)
                    candidate_affinity = self._compute_candidate_affinity_for_cell(row, cell)
                    ...
                    cell_data = {
                        ...
                        'candidate_affinity': candidate_affinity,  # NEW
                    }
    ...
    def prepare_features(self, cells_data, fit_transform=True):
        """Prepare features and candidate affinities for model input."""
        ...
        for cell in cells_data:
            ...
            # NEW: candidate affinity per party for this cell
            if 'candidate_affinity' in cell:
                cand_dict = cell['candidate_affinity']
                cand_vec = np.stack(
                    [np.array(cand_dict[party], dtype=np.float32)
                     for party in self.party_names],
                    axis=0
                )  # [K, D]
            else:
                cand_vec = np.zeros(
                    (len(self.party_names), getattr(self, 'candidate_feature_dim', 3)),
                    dtype=np.float32
                )
            candidate_affinities.append(cand_vec)
        ...
        # Stack candidate affinities and standardize across (cell, party)
        if len(candidate_affinities) > 0:
            candidate_affinities = np.stack(candidate_affinities, axis=0)  # [C, K, D]
        else:
            candidate_affinities = np.zeros(
                (0, len(self.party_names), getattr(self, 'candidate_feature_dim', 3)),
                dtype=np.float32
            )

        if fit_transform:
            flat = candidate_affinities.reshape(-1, candidate_affinities.shape[-1])
            self.candidate_mean = flat.mean(axis=0)
            self.candidate_std = flat.std(axis=0) + 1e-6
        else:
            if not hasattr(self, 'candidate_mean'):
                raise ValueError("Candidate scalers not fitted; run with fit_transform=True on train first.")
        candidate_affinities = (candidate_affinities - self.candidate_mean) / self.candidate_std
        ...
        return {
            ...
            'candidate_features': torch.FloatTensor(candidate_affinities),  # NEW
...
def compute_loss_booth_batch(model, data_dict, booth_batch, booth_metadata, 
    ...
    """Compute loss for a batch of booths with candidate term support.
    ...
    # NEW: candidate features subset
    batch_candidate = None
    if 'candidate_features' in data_dict:
        batch_candidate = data_dict['candidate_features'][mask]

    # Forward pass with candidate features
    turnout_probs, party_probs, party_log_probs = model(
        batch_features, batch_booth_indices, candidate_features=batch_candidate
    )

    booth_losses = []

    unique_booth_indices = torch.unique(batch_booth_indices)
    for booth_idx in unique_booth_indices:
        booth_idx_int = booth_idx.item()
        booth_info = booth_metadata.get(booth_idx_int)
        ...
        T_i = booth_info['T_i']
        t_i = booth_info['t_i']
        p_i = torch.tensor([booth_info['p_i'][party] for party in model.party_names],
        ...
        V_hat = torch.sum(
            booth_cell_weights.unsqueeze(1) * booth_turnout_probs.unsqueeze(1) * booth_party_probs,
            dim=0
        )
        ...
        bce_loss = -(t_i_tensor * torch.log(t_hat + 1e-12)
                     + (1 - t_i_tensor) * torch.log(1 - t_hat + 1e-12))
    ...
    # NEW: L2 regularization for η
    if hasattr(model, 'eta') and model.eta is not None:
        reg_loss = reg_loss + lambda_eta * torch.sum(model.eta ** 2)
...
def train_model_with_early_stopping(model, train_data, val_data, train_booth_indices, 
    ...
    # Move data to device (including candidate features)
    for key in ['features', 'candidate_features', 'cell_weights', 'booth_indices']:
...
def validate_and_calibrate(model, data_dict, booth_indices, booth_metadata, party_names):
    ...
    with torch.no_grad():
        # Get predictions for all booths (including candidate features if available)
        cand_features = data_dict.get('candidate_features', None)
        turnout_probs, party_probs, _ = model(
            data_dict['features'],
            data_dict['booth_indices'],
...
def extract_coefficients(model, feature_names, party_names):
    ...
    # Candidate affinity coefficients (η) for [age_sim, religion_match, caste_match]
    if hasattr(model, 'eta') and model.eta is not None:
        eta_vec = model.eta.detach().cpu().numpy().tolist()
        coefficients['candidate_eta'] = {
            'age_similarity': float(eta_vec[0]) if len(eta_vec) > 0 else 0.0,
            'religion_match': float(eta_vec[1]) if len(eta_vec) > 1 else 0.0,
            'caste_match': float(eta_vec[2]) if len(eta_vec) > 2 else 0.0,
        }
...
def main(file_2020=None, file_2025='Assembly_Madipur_2025_Election.xlsx', 
    ...
    # OPTIONAL: Hardcode candidate info here instead of using Excel columns.
    # Uncomment and edit the values for your assembly's candidates.
    # Example:
    # candidate_profiles = {
    #     'BJP': {
    #         'age': 43,          # integer age or age band like 'Age_46-60'
    #         'religion': 'hindu',
    #         'caste': 'vaishya',
    #     },
    #     'Congress': {
    ...
    #     },
    # }
    # processor.set_global_candidate_profiles(candidate_profiles)
    ...
    candidate_dim = getattr(processor, 'candidate_feature_dim', 0)

    print(f"Model architecture: {n_features} features, {n_parties} parties, {n_booths} booths, candidate_dim={candidate_dim}")
    
    model = AggregateVotingModel(n_features, n_parties, n_booths, candidate_dim=candidate_dim, device=device)
    model.to(device)
    ...
    # Print final candidate affinity coefficients (η)
    if hasattr(model, 'eta') and model.eta is not None:
        eta_vec = model.eta.detach().cpu().numpy()
        if eta_vec.shape[0] >= 3:
            print("\nFinal candidate affinity coefficients (η):")
            print(f"  Age similarity (g_age):     {eta_vec[0]: .6f}")
            print(f"  Religion match (g_relig):  {eta_vec[1]: .6f}")
            print(f"  Caste match (g_caste):     {eta_vec[2]: .6f}")
...
def quick_validation_test():
    ...
    try:
        # Load 2025 data
        df_2025 = pd.read_excel('Madipur_aggregate.xlsx')
...
if __name__ == "__main__":
    ...
    if quick_validation_test():
        ...
        model, coefficients, predictions, validation, training_info = main(
            file_2020=None,
            file_2025='Madipur_aggregate.xlsx',
